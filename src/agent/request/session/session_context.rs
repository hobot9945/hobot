...
impl SessionContext {
    ...
    fn _build_report(&self, payload: &SessionContext) -> Result<(), AgentError> {
        ...
        body.push_str("Для загрузки нужно вывести следующий текст (подставив актуальный \
session_id), включая забор из бэктиков:\n\
Открытие новой сессии с другим session_id **НЕ** является началом \
