...
ошибка: {}"#, file!(), line!(), &enumerate_lines(&json_body, Some(1)), e))})?;
...
impl SessionContext {
    ...
    fn _build_report(&self, payload: &SessionContext) -> Result<(), AgentError> {
        ...
        body.push_str("Открытие новой сессии с другим session_id **НЕ** является началом \
нового чата и **НЕ** изменяет контекст. Промпт **НЕ** должен загружаться повторно. \
Для загрузки нужно вывести следующий текст (подставив актуальный session_id), включая забор из бэктиков:\n");
