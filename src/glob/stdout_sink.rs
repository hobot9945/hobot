...
//!
//! # ОТВЕТСТВЕННОСТЬ
//! - Формирование сообщений о критических ошибках в соответствие с протоколом.
//! - Обертка JSON-полезной нагрузки в текстовые маркеры Хобота (`<<<hbt ... >>>hbt`).
//! - Делегирование физической отправки функции `glob::send_raw_to_stdout`.
//!
//! # ПРОТОКОЛ
...
pub fn send_critical_to_stdout(msg: &str) {
    ...
    let json_body = serde_json::to_string(&payload).unwrap_or_else(|_| {
        r#"{"type":"CRITICAL_ERROR","error_message":"Serialization failed"}"#.to_string()
    });
