...
class DirectiveExtractor {
    ...
    _moveLastDirectiveToSchlich() {
        ...
        while (true) {
            ...
            // 4.1 Проверяем, не является ли директива дубликатом предыдущей. Для скорости, сначала сравниваем длины
            // старой и новой директив.
            //
            // Здесь хитрость. Некоторые сайты режут длинные строки в окнах вывода. Я расширяю синтаксис JSON, разрешаю
            // использовать обратный слэш и перевод строки, как в языках программирования. // Я прошу AI добавлять
            // после \n последовательность \0x0A, которая свернет строчку в окне, превратив текст в многострочный. А
            // здесь я \0x0A вырезаю, снова приводя его к нормальному JSON.
//            const directive = buf.substring(prevTag.index, mainClose.endIndex) + "\n";        // было раньше
            const directive =
                buf.substring(prevTag.index, mainClose.endIndex).replace(/\\\r?\n/g, "") + "\n";
