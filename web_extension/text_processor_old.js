...
class TextProcessor {
    ...
    /**
     * Ищет ближайший контейнер code-block для узла (сам или вверх по дереву).
     *
     * Контракт:
    ...
    collectCodeBlocks(node, blocksToProcess) {
        ...
        // 1. Поиск вверх: узел находится внутри code-block или сам является им
        const container = this.getCodeBlockContainer(node);
        if (container) {
            if (!this.isGarbageNode(container)) {
                blocksToProcess.add(container);
            }
            return; // Контейнер найден, спускаться не нужно
        }

        // 2. Поиск вниз: узел содержит code-block внутри себя
        if (node.nodeType === Node.ELEMENT_NODE) {
            const innerBlocks = node.querySelectorAll('pre');
            innerBlocks.forEach(block => {
                if (!this.isGarbageNode(block)) {
    ...
    initializeObserver() {
        ...
        this.observer = new MutationObserver((mutations) => {
            ...
            // Зафиксировать тип проведенного для directive_extractor.js парсинга.
            this.dirExtractor.webParsingType = window.Globals.webSiteCodeBlocksWrapping.PRE;

            // Предать экстрактору выделенный текст.
