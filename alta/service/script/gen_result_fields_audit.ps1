...
for ($i = 0; $i -lt $lines.Count; $i++) {
    ...
    if ($State -eq "OutOfTable") {
        ...
        if ($isTableLike) {
            ...
            # Если предыдущая таблица завершилась, но так и не была закрыта аудитом перед началом новой:
            if ($inAuditBlock) {
                Add-Error $lnNo "Таблица была завершена без маркера _audit/_item_audit."
                # Мягкий сброс состояния, чтобы изолировать ошибку и не ломать проверку новой таблицы
                $inAuditBlock = $false
                $blockRowCount = 0
                $expectedNum = 1
            }
