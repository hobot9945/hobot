...
- 06: transport.border_id:
  - VR: если авиаперевозка → номер рейса (приоритет: formalized.awb.GoodsMovement → non_formalized.svh_1.flight_number); если автоперевозка → пусто (в базовом варианте)
  - S: CP | pending
...
- Если группировка товаров запрещена (решение 257, см. `aggregation_forbidden_list`):
...
### 5.3 `aggregation_forbidden_list` - Товары, запрещенные к группировке
...
### 5.5 Проверка скриптами полностью/частично сформированного файла `dt_fields.md`
...
### 5.6 Проверка на наличие статусов pending
...
### 5.7 Атомарность элементов массива (goods[])
...
### 5.8 Комплектность разделов товара
...
### 6.2 Подготовка
  ...
  2. **ОСТАНОВИ РАБОТУ И ВЫЙДИ В ДИАЛОГ**, чтобы, совместно с оператором, решить, входит ли поставка в 
     `aggregation_forbidden_list` (запрещена ли группировка).
...
сгенерируй/доработай `dt_fields.md`:
  ...
  - Наполнение массива `goods[]` веди строго по принципу атомарности (п. 5.7) и комплектности (п. 5.8).
...
#### 6.4.2 В цикле генерации:
  ...
  - Перед выполнением каждой директивы записи/дозаписи `dt_fields.md`:
    ...
    - ✅ Перед переходом к `goods[i+1]` убедись, что текущий `goods[i]` полностью укомплектован (п. 5.8).
