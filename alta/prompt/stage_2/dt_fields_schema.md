...
Вход этапа:
  ...
  - `alta\prompt\codebook.md` - полные справочники. Большие, читать только по необходимости, **НЕ ЧИТАТЬ ЗАРАНЕЕ**.
  - `alta\prompt\dt_rules.md`
...
Правила определения типа перевозки:
- Авиаперевозка = если в primary.md присутствует formalized.awb (авианакладная).
- Автоперевозка = если в primary.md присутствует formalized.cmr (CMR).

- 01: transport.vehicles_count:
  - VR: если авиаперевозка → пусто; если автоперевозка → число блоков TransportMeans_* в formalized.packing_list
  - S: D | pending
  - N: графа 18 — количество транспортных средств (G_18_0). Для авиаперевозки не заполняется.

- 02: transport.identification:
  - VR: если авиаперевозка → пусто; если автоперевозка → приоритет источников: join(formalized.packing_list.TransportMeans_*.Number, "/") → non_formalized.td_1.transport_reg_number → non_formalized.svh.transport_reg_number
  - S: D | pending
  - N: графа 18 — идентификация ТС (G_18). Для авиаперевозки не заполняется.

- 03: transport.registration_country_code:
  - VR: если авиаперевозка → пусто; если автоперевозка → если formalized.packing_list.TransportMeans_1.NationalityCode = "000" → "00", иначе взять как есть
  - S: D | pending
  - N: графа 18 — код страны регистрации ТС (G_18_2). Для авиаперевозки не заполняется.
...
- 05: transport.border_mode:
  - VR: 1 (константа для обоих видов транспорта)
  - S: D | pending
  - N: графа 21 — количество активных ТС на границе (G_21_0)

- 06: transport.border_id:
  - VR: если авиаперевозка → номер рейса (formalized.awb.flight_number); 
    если автоперевозка → пусто (в базовом варианте)
  - S: CP | pending
  - N: графа 21 — номер авиарейса / идентификация ТС на границе (G_21)

- 07: transport.border_country_code:
  - VR: если авиаперевозка → "00"; если автоперевозка → пусто
  - S: D
  - N: графа 21 — код страны регистрации ТС на границе (G_21_2)
...
- 01: shipment.transaction_nature:
  - VR: 010
  - S: D
  - N: графа 24 — характер сделки (G_28_21)

- 02: shipment.transaction_feature:
  - VR: 00 (если есть master_data.unk), иначе 06
  - S: D
  - N: графа 24 — особенность сделки (G_28_12)
...
- 01: transport.border_transport_code:
  - VR: если авиаперевозка → 40; если автоперевозка → 31
  - S: D
...
- 01: customs.border_code:
  - VR: non_formalized.td.customs_post_code для авто поставки; formalized.awb.customs_post_code для авиа поставки
  - S: CP | pending
  - N: графа 29 — код таможенного органа на границе (G_29_1); источник: ТД (09013)

- 02: customs.border_name:
  - VR: non_formalized.td.customs_post_name для авто поставки; formalized.awb.customs_post_name для авиа поставки
  - S: CP | pending
  - N: графа 29 — наименование таможенного поста на границе (G_29_2); источник: ТД (09013 для авто или 02017 для авиа)
...
- 01: location.document_number:
  - VR: non_formalized.svh.warehouse_license_number для авто поставки; formalized.awb.warehouse_license_number для 
    авиа поставки
  - S: CP | pending
...
### 3.16 Массив товаров goods[] (BLOCK, графы 31–47)
- Описание структуры массива товаров goods:
  ...
  - Товар представляется элементов массива товаров и может состоять из одной или, если разрешена группировка по коду ТН ВЭД, из 
    нескольких товарных позиций. Товарные позиции представляются подмассивами. Если есть только одна товарная позиция,
    подмассив состоит из одного элемента.
  - Первоначально товарные позиции представлены плоским массивом `invoice.InvoiceGoods[*]` в `primary.md`, но каждый 
    его элемент содержит два поля, `dt_item_index` (индекс элемента в массиве товаров) и `dt_tovg_index` 
    (индекс в массиве позиций товаров). Эти поля облегчают группировку, если она разрешена.
  - **ВНИМАНИЕ! ЗАПРЕЩЕНО СЧИТАТЬ НАЛИЧИЕ ПОЛЕЙ `dt_item_index`, `dt_tovg_index` РАЗРЕШЕНИЕМ ГРУППИРОВКИ**. Это
    вспомогательные поля, они **НЕ** определяют стратегию расчета.
  - Описание элемента массива товаров `goods[i]` разбито на 
    - блоки полей `3.16.1 Графа 31`, `3.16.2 Графы 32–38`
    - и подмассив позиций с одним `tnved_code`, его элементы состоят из: 
      - текстового описания позиции `goods[i].txt[j]`, `3.16.4 дополнение к графе 31`
      - характеристики позиции `goods[i].tovg[j]` (набора полей), `3.16.6 дополнение к графе 31`.
  - подмассив goods[i].g44_docs[k] - массив полей, описывает документы, относящиеся к товару [i].

- Если группировка товаров запрещена (решение 257, см. `dt_rules.md`/`aggregation_forbidden_list`):
  - элемент массива `goods[i]` имеет одну позицию `goods[i].txt[1]`/`goods[i].tovg[1]`.  

- Если группировка разрешена:
  - элемент массива `goods[i]` имеет подмассивы:
    - goods[i].txt[j] - строковая переменная,
    - goods[i].tovg[] - массив полей.
...
- Материализация этого раздела в `dt_fields.md`:
   - представлен заголовком `### 3.16 Массив goods[]`,
   - имеет служебное поле `_array_audit: N`.
...
#### 3.16.0 Элемент массива товаров goods[i]
...
Часть полей элемента массива goods[i], для аудита выглядит как обычный раздел. То же 3.16.2, 3.16.3.
...
- 08: goods[i].procedure_code:
  - VR: declaration.procedure.value + "00" + "000"
  - S: D
  - N: графа 37 — код процедуры по товару (G_37_1)
...
- 10: goods[i].supplementary_quantity:
  - VR: сумма goods[i].tovg[*].quantity
  - S: D | pending
  - N: графа 41 — количество в дополнительной единице измерения (G_31_8)

- 11: goods[i].supplementary_unit_code:
  - VR: goods[i].tovg[*].unit_code (если одинаковый, иначе pending)
  - S: D | pending
  - N: графа 41 — код дополнительной единицы измерения (G_41_1)

- 12: goods[i].supplementary_unit_name:
  - VR: goods[i].tovg[*].unit_name (если одинаковый, иначе pending)
  - S: D | pending
  - N: графа 41 — наименование дополнительной единицы измерения (G_41_2)
...
- 01: goods[i].invoice_cost:
  - VR: сумма goods[i].tovg[*].invoice_cost 
  - S: D | pending
...
#### 3.16.4 Дополнение к графе 31 — TXT (детальные строки)
...
Материализация:
```dt_fields
#### 3.16.4 Массив: goods[i].txt[]
- _array_audit: <N>
```

#### 3.16.5 Элемент массива goods[i].txt[j]
Материализация:
```dt_fields
#### 3.16.5 Элемент массива: goods[i].txt[j]
- _element_num: <j>
...
- 01: goods[i].txt[j]:
  - VR: `"АРТ: - " + goods[i].tovg[j].quantity + " " + goods[i].tovg[j].unit_name` + " " + `goods[i].tovg[j].description`
  - S: D | pending
  - N: графа 31 — TXT строка 1 (арт/кол-во/ед)

- _item_audit: 1

#### 3.16.6 Таблица описания — TOVG
`goods[i].tovg[]` — массив групп товаров, графа 31 (таблица). См. правило маппинга, п. 3.17.4.

Материализация:
```dt_fields
#### 3.16.6 Массив: goods[i].tovg[j]
- _array_audit: <N>
```
...
В графу 44 включаются все документы, имеющие признак `doc_gr44: true`. Список документов повторяется для каждой товарной
позиции, но Альта требует этой избыточности. Сокращения **ЗАПРЕЩЕНЫ**.
...
Материализация:
```dt_fields
#### 3.16.10 Массив: goods[i].g44_docs[k]
- _array_audit: <N>
```

#### 3.16.11 Элемент массива goods[i].g44_docs[k]
- Для каждого документа, имеющего поле `doc_gr44: true` - одна запись `goods[i].g44_docs[k]`. Если поля пустые,
  не помещай документ в список.
- ИСКЛЮЧЕНИЕ: в массиве разрешительных документов `non_formalized.regulatory_docs` (если есть), каждой модели товара 
  соответствует группа документов (например, сертификат соответствия (EAC), нотификация (NTF), 
  сертификат происхождения (CO)). Если поля для документа пустые, не помещай документ в список. Иначе, 
  для документа создается запись `goods[i].g44_docs[k]`.
...
### 5.6 Атомарность элементов массива (goods[])
При записи массива частями (чанками), каждый элемент `goods[i]` должен быть записан монолитно и полностью (все 
подразделы от 3.16.1 до 3.16.11) перед тем, как будет начат следующий товар `goods[i+1]`. Перемешивание (интерливинг) 
разделов разных товаров в файле строго запрещено.

### 5.7 Комплектность разделов товара
Один элемент товара `goods[i]` считается завершенным только тогда, когда материализованы все обязательные таблицы:
- 3.16.1 (G31 поля)
- 3.16.2 (G32-38 поля)
- 3.16.3 (Стоимость поля)
- 3.16.4/5 (TXT массив)
- 3.16.6/7 (TOVG массив)
- 3.16.8/9 (G44 Text поле)
- 3.16.10/11 (G44 Docs массив)

---

## Раздел 6. Алгоритм: **ДЕТЕРМИНИРОВАННО (СТРОГО ПО ПУНКТАМ)**

### 6.1 Разведка
  1. Вспомни ограничения сайта.
  2. Если уже существует `dt_fields.md`, `dt_fields_review.md`, запроси у оператора нужна ли доработка или генерация с нуля?

### 6.2 Подготовка
  1. Прочитай `primary.md` одной командой Хобота (если позволяют ограничения сайта).
  2. **ОСТАНОВИ РАБОТУ И ВЫЙДИ В ДИАЛОГ**, чтобы, совместно с оператором, решить, входит ли поставка в
     `dt_rules.md`/`aggregation_forbidden_list` (запрещена ли группировка).
...
сгенерируй/доработай `dt_fields.md`:
  ...
  - При каждой записи `dt_fields.md`, целиком/части/завершении, **в этой же директиве AI ОБЯЗАН ЗАПУСТИТЬ ПРОВЕРОЧНЫЙ
    СКРИПТ `gen_result_fields_audit.bat`**, см. п. 5.5.

  - Используй patch_file, для точечных фиксов.
   
  - Наполнение массива `goods[]` веди строго по принципу атомарности (п. 5.7) и комплектности (п. 5.7).
...
#### 6.4.2 В цикле генерации:
  ...
  - Перед выполнением каждой директивы записи/дозаписи `dt_fields.md`:
    ...
    - ✅ Перед переходом к `goods[i+1]` убедись, что текущий `goods[i]` полностью укомплектован (п. 5.7).
...
| 02017  | Авиационная накладная (Air Waybill) |
