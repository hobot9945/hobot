...
3. Часть 3. Формат `dt_fields.md`

4. Часть 4. Правила работы

5. Часть 5. Порядок работы (задание)
...
- Правило агрегации единиц в товар (строго):
  ...
  - сгруппировать строки по `GoodsCode`; `GoodsCode` = значение `invoice.InvoiceGoods_*.GoodsCode` 
...
`goods[i].tovg[]` — массив групп товаров, графа 31 (таблица).
...
Пример для вложенных массивов:
```dt_fields
#### 3.16.7 Элемент массив: goods[i].tovg[j]
- _element_num: <j>
пустая строка
...
- Скрипт `alta\service\script\gen_result_fields_audit.bat alta\stage_2.0_result\<case>\dt_fields.md` проверит:
...
- Скрипт `alta\service\script\gen_result_full_audit.bat alta\stage_2.0_result\<case>\dt_fields.md`. Он:
...
### 6.4.3 Завершение
  - ✅ Запусти полную проверку `gen_result_full_audit.bat` (кроме аудита полей выполнит аудит массивов).
