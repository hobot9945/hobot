...
2. Раздел 2. Правила шаблонов

3. Раздел 3. Шаблоны полей ДТ

3. Раздел 3. Формат `dt_fields.md`

4. Раздел 4. Правила работы

5. Раздел 5. Порядок работы (задание)

6. ПРИЛОЖЕНИЕ. Вырезки из справочников
...
- `dt_fields.md` - это полный набор полей ДТ по этой схеме, не просто текстовый документ, а формальная база данных 
  для этапа 2.1. Если отсутствует хотя бы одно поле из раздела "Шаблоны полей ДТ" — это ошибка этапа. AI обязан 
  фиксировать все пробелы/конфликты в разделе `issues` файла `dt_fields.md`, откуда они попадут в `dt_fields_review.md`

---

## Раздел 2. Правила шаблонов
...
- 03: goods[i].transport_to_border:
  - VR: goods[i].invoice_cost_in_rub + goods[i].transport_to_border + goods[i].insurance_to_border
    где:
    - goods[i].invoice_cost_in_rub = goods[i].invoice_cost * shipment.currency_rate,
    - goods[i].transport_to_border = доля товара i в расходах на транспорт до границы ЕАЭС,
...
`goods[i].txt[]` — подмассив строк дополнения к графе 31 (описание товара).
Правило: `txt[j]` соответствует `tovg[j]` (1:1), т.е. число элементов `txt[m]` равно числу элементов `tovg[m]`,
...
#### 3.17.5 Таблица описания — TOVG
`goods[i].txt[]` — подмассив групп товаров, графа 31 (таблица).
...
### 4.2 Таблицы полей
Далее идут таблицы:
  - для каждого раздела/массива строится отдельная таблица;
...
### 4.4 После каждого раздела (НЕ массива):
...
### 4.5 Для массивов
Индексы массивов (`goods[i]`, `goods[i].txt[j]`, `goods[i].tovg[j]`, `goods[i].g44_docs[k]`) **ВСЕГДА 1-BASED**. 

#### 4.5.1 Заголовок массива

```primary
#### Массив: <ArrayName>[N] (где N - размерность массива)
```

#### 4.5.2 Заголовок элемента массива
**!!!ВАЖНО ДЛЯ СКРИПТА ПРОВЕРКИ!!!** - выводится опорная строка:
...
Пример для массива (описание товаров (G_31), goods[]):
```primary
- _item_audit: 4
- _array_audit: 2
```
...
### 5.4 Проверка скриптом `check_stage_result.bat` полностью/частично сформированного файла`dt_fields.md`
...
Важно: скрипт корректно проверит только файл (полный или незаконченный), который завершен строкой `_audit`/`_item_audit`. 
...
сгенерируй/доработай `dt_fields.md`:
  ...
  - При каждой записи `dt_fields.md`, целиком/части/завершении, **в этой же директиве AI ОБЯЗАН ЗАПУСТИТЬ ПРОВЕРОЧНЫЙ
    СКРИПТ `check_stage_result.bat`**, см. п. 5.4.
...
### 6.4.3 Завершение
  - ✅ проверь, что были материализованы **ВСЕ** разделы шаблонов ДТ из схемы.
  - Сгенерируй `primary_review.md`.
  - Совместно с оператором разреши `pending`, если есть.
