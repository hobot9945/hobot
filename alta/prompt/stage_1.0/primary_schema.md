...
получение от оператора, недостающих данных и разрешение конфликтов. Результатом работы является 
файл `primary.md`, который служит единственным источником данных для этапа 1.1 (формализация) и этапа 2.0 (поля ДТ).
...
- ✅ Никакие существенные данные из первички не потеряны. Если есть данные, которые, не могли быть помещены в
  `primary.md`, они должны быть зафиксированы в `primary_review.md` с указанием причины, например, "нет подходящего шаблона".
...
Сгенерировать и проверить `primary.md`, заново выполнив пункт 3.

### Примечание:

- Для AI **РАЗРЕШЕНА** запись в `alta\stage_1.0_result\<кейс>\...`, файлы `primary.md`, `primary_review.md` пишутся
  в этот каталог.
