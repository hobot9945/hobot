...
Пример: запрещено создавать в `primary.md` описание для `Supplementary Contract` и заполнять его поля пустыми значениями
или полями других документов, если не взято ни одного поля из исходного документа.

Примечание: “Запрет симуляции документа” означает запрет создавать **несуществующий документ** или переносить в него факты
из других документов “для заполнения”.  

Но, если документ **реально найден**, то **каркас структуры и все поля по шаблону обязаны быть материализованы**,
с обязательным `pending` для полей, которые не удалось извлечь.
...
0. **ЗАПРЕЩЕНА ПРЕДВАРИТЕЛЬНАЯ/БЫСТРАЯ/СОКРАЩЕННАЯ СБОРКА `primary.md`!!! СБОРКА ВСЕГДА ДОЛЖНА БЫТЬ ПОЛНОФУНКЦИОНАЛЬНОЙ.**
...
1. **Нормализация и полнота структуры:** `primary.md` — формальная база данных.  
   ...
   в `primary.md`, даже если `value` пусто и `status: pending`.  
   Если массив, есть в шаблоне, но в документе нет/не достаточно данных — массив всё равно материализуется согласно
   правилам конкретного шаблона (например, SVH goods — минимум 1 pending-элемент).
...
### 1. Прочитать / убедиться, что структура следующих каталогов, включая размеры файлов, известна. 
  ...
  - `alta\source\<кейс>\...\<папка первичных документов>\operator\` (Опционально. Данные, предоставленные оператором. Каталог может 
    отсутствовать / быть пустым, если диалога с оператором не было.)
   
### 2. Прочитай все исходные файлы.
Прочитай из каталога `md`: `DOC_CONVERSION_REVIEW.md` и все md-файлы. Читай файлы целиком.

### 3. Сгенерируй или подхвати готовый `primary.md`.
- Если уже есть сгенерированные `primary.md`, `primary_review.md`, выйти в режим диалога,
- иначе, сгенерировать их.
 
**ЖЕСТКОЕ ПРАВИЛО:**
Поля в схеме идут с возрастающими на единицу номерами. В `primary.md` генерируй их с теми же номерами, номер **ВСЕГДА**
возрастает на единицу. После генерации каждого поля:
- **!!!ПРОВЕРЬ ФАКТ ВОЗРАСТАНИЯ НОМЕРА НА 1!!!**. Проверь, также, совпадение имени поля со схемой.
- Проверку возрастания на 1 делай и для полей документа, и для номеров массивов (в заголовках вида #### NN: Array[]), 
  и внутри каждого элемента массива (01.._item_audit).

После обработки документа:
