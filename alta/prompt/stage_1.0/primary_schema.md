...
0. **ЗАПРЕЩЕНА ПРЕДВАРИТЕЛЬНАЯ/БЫСТРАЯ/СОКРАЩЕННАЯ СБОРКА `primary.md`!!! СБОРКА ВСЕГДА ДОЛЖНА БЫТЬ ПОЛНОФУНКЦИОНАЛЬНОЙ.**
   Не используй плейсхолдеры: write_file один раз целиком, либо write_file + append. Используй patch_file только после 
   сборки, для точечных фиксов.
...
- **Неформализуемые поля:**
  - 47: `transport_to_border` (стоимость перевозки до границы)
