...
0) **Фиксация версии и контекста**
- ✅ `meta.case_name`, `meta.shipment_path`, `meta.generated_at` заполнены и соответствуют текущему кейсу/папке поставки.

1) **Исходники реально прочитаны**
- ✅ в `primary_review.md` перечислен список файлов-источников, фактически использованных для заполнения `primary.yaml`
  (минимум: имя файла + роль: contract/invoice/pl/cmr/svh/td/master_data/operator).
...
- ✅ все документы, которые **присутствуют в первичке** и для которых есть шаблоны в этом промпте, присутствуют в 
  `formalized`/`non_formalized`.
- ✅ обязательные для автоперевозки документы присутствуют: `Contract`, `Invoice`, а при наличии признаков 
  автоперевозки — `CMR`.

3) **Полнота полей по каждому документу**
- ✅ для каждого документа материализованы **все поля**, перечисленные в его шаблоне (разделы 6.* и 7.*), даже если 
  `status=pending`.
- ✅ для массивов (`InvoiceGoods`, `PackingList.Goods`, `CMRGoods`, `non_formalized.svh.goods`) материализованы элементы 
  `1..N` согласно первичке (не агрегированы и не потеряны).
...
5) **(Полный режим) Проверка `primary.md`**
- ✅ `primary.md` является представлением `primary.yaml`: не содержит уникальных фактов.
- ✅ в `primary.md` по каждому документу есть таблица со всеми полями шаблона, включая `pending`, и раздел вопросов 
  соответствует `issues` из `primary.yaml`.
