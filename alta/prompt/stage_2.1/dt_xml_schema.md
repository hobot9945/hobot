...
Вход этапа 2.1:
- **ТОЛЬКО:** `alta\stage_2.0_result\<case>\dt_fields.md`
- другие источники данных не используются

Выход этапа 2.1:
- `alta\stage_2.1_result\<case>\dt.xml` (или `<case>_dt.xml`) в кодировке `windows-1251`.
- `alta\stage_2.1_result\<case>\dt_xml_review.xml` (или `<case>_dt.xml`) в кодировке `windows-1251`.

## 1. Исполнение задачи

### 1.1 Базовые принципы преобразования

  1) **Whitelist-теги:** генератор выводит **только** теги и структуры, перечисленные в разделе «2) Структура и маппинг». 
