...
### 1.1 Правила
  ...
  3) **Даты:** Приведение к формату `YYYY-MM-DD`.

  4) **Пустые XML поля:** если поле в `dt_fields.md` имеет `status: pending` — оставь в XML пустое значение, фиксируй факт
     в `dt_xml_review.md` и сообщи оператору.
...
При автоперевозке теги `G_18_0`, `G_18`, `G_18_2` и `G_21_0` генерируются обязательно.
При авиаперевозке теги `G_18_0`, `G_18` и `G_18_2` **не генерируются**. Генерируются теги `G_21_0` (со значением `1`), 
`G_21` (рейс) и `G_21_2` (код страны `00`).

| XML тег  | dt_fields path                              | Комментарий |
|----------|---------------------------------------------|-------------|
| `G_18_0` | `transport.vehicles_count.value`            | только для авто |
| `G_18`   | `transport.identification.value`            | только для авто |
| `G_18_2` | `transport.registration_country_code.value` | только для авто |
| `G_19_1` | `transport.container_flag.value`            | для всех |
| `G_21_0` | `transport.border_mode.value`               | для всех (всегда `1`) |
| `G_21`   | `transport.border_id.value`                 | только для авиа (рейс) |
| `G_21_2` | `transport.border_country_code.value`       | только для авиа (страна `00`) |
...
**ВНИМАНИЕ:** ширина поля Альты 64 символов. Для сворачивания текста вставляй переводы строк `&#10;`. В конце строк 
вставляй `&#13;&#10;`
...
Правило:
- каждый элемент массива `goods[i].g44_docs[]` → отдельный узел `<G44>...</G44>`.

Маппинг:

| XML тег внутри `G44` | dt_fields path (для goods[i].g44_docs[k]) | Комментарий |
|----------------------|-------------------------------------------|-------------|
| `G4403`              | `kind_code.value`                         | признак записи; если `status: pending` — тег не генерировать |
| `G441`               | `doc_code.value`                          | код вида документа |
| `G442`               | `doc_number.value`                        | номер документа |
| `G442R`              | `dt_number.value`                         | номер ДТ, с которой подавался документ (если kind_code = 2) |
| `G443`               | `doc_date.value`                          | дата `YYYY-MM-DD` |
| `G444`               | `doc_name.value`                          | наименование документа |
