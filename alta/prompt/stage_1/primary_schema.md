...
Полное извлечение фактов из папок `alta\source\<каталог поставки>\` и `alta\stable_source\`, а также получение в чате, 
путем диалога с оператором, недостающих данных и разрешение конфликтов. Результатом работы является файл `primary.md`, 
...
В зависимости от того, выполнялся ли этап 0, источники данных могут отличаться. Этап 0 конвертирует различные форматы 
первичных документов в маркдаун, `alta\source\<каталог поставки>\md\`. Если такие файлы существуют, нужно использовать
их, поскольку это сильно облегчает AI загрузку данных. Если этих файлов нет, нужно использовать первичные документы.
Если в процессе работы обнаружится, что в файлах из `md` не отражена какая-либо информация, ее следует искать в первичных
документах, а факт отсутствия данных зафиксировать в `review_1.md`.
...
6. **Сохранение полноты:** Формализация не должна приводить к потере информации.
  - Реквизиты документов максимально раскладываются по полям.
...
8. На этапе 1 в рабочем режиме AI не имеет права использовать как источник фактов:
...
9. **Предпочтение русских названий:** если документ (например, контракт) продублирован на русском и английском языках, 
   то формализовывать следует русскую версию.
...
Для Этапа 2 логический путь к полю строится так:
...
Для формализуемых документов идентификатор поля совпадает с xml тегом. Это camel case. Для неформализуемых
...
**AI обязан материализовать все поля, указанные в шаблоне документа:** Если значение отсутствует - использовать `pending`.

**При материализации поля оформляются таблицей**
...
- **Ключевые поля:**
  - `number`
  - `date`
  - `warehouse_license_number`: (номер лицензии СВХ)
  - `warehouse_license_date`: (дата лицензии СВХ)
