...
- **Товары в разрезе кодов ТН ВЭД,** (поле `GoodsCode` для позиций с `dt_item_index` = k):
  - `goods_[k]`
    - 01: `tn_ved` (ТН ВЭД)
    - 02: `description` (формируется на основе расшифровки ТН ВЭД и тех. описания, см. раздел `Правила сборки`, п. 7.6)
...
### 8.4 Завершение
  1) **ПРОВЕРЬ ПРАВИЛЬНОСТЬ ПЕРЕНОСА ДАННЫХ**, то есть, что нет рассинхрона в перенесенных полях.
