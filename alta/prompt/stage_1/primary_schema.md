...
### 1.1 Вход этапа:
- `alta\source\<ИмяКейса>\<номер_поставки>\<папка первички>\md\*.md` - факты поставки (приведенная к `md` первичка),

- **ИЛИ** оригинальные первичные документы из `alta\source\<ИмяКейса>\<номер_поставки>\<папка первички>\*` 
  (если папка `md` отсутствует или оператор указал работать напрямую),

- `alta\source\<ИмяКейса>\master_data.md` - данные, мало изменяющиеся между кейсами (declarant, representative, 
  letter_of_attorney, contract, supplementary_contract, unk, transport_contract, exemption_letter),
...
**ОЕБЕСПЕЧЬ СОГЛАСОВАНИЕ ВЕСОВ С УПАКОВОЧНЫМ ЛИСТОМ.**
...
- **Поля шапки / реквизиты документа:**
  ...
  - 04: `PlacesDescription` (описание мест, напр. "Поддон"; для сверок/контекста)
  - 05: `GrossWeightQuantity` (общий вес брутто; для сверок с CMR/PL/СВХ)
  - 06: `NetWeightQuantity` (общий вес нетто; для сверок)
  - 07: `GCost` (системное поле Альты; дубль `TotalCost`; для импорта/совместимости; derived)
  - 08: `TotalCost` (полная стоимость; источник для графы 22: G_22_2)

  - 09: `DeliveryTerms_DeliveryPlace` (место поставки по Incoterms; источник для графы 20: G_20_2)
  - 10: `DeliveryTerms_DeliveryTermsStringCode` (строковый код условий, напр. `EXW`; источник для графы 20: G_20_1)
  - 11: `DeliveryTerms_DispatchCountryCode` (страна отправления alpha-2; источник для графы 15A: G_15A_1)
  - 12: `DeliveryTerms_TradingCountryCode` (торгующая страна alpha-2; источник для графы 11: G_11_1)
  - 13: `DeliveryTerms_DestinationCountryCode` (страна назначения alpha-2; источник для графы 17A: G_17A_1)

  - 14: `Registration_PrDocumentName` (наименование документа для печати/сверок; может использоваться в графе 44: G44/G444)
  - 15: `Registration_PrDocumentNumber` (номер инвойса; графа 44: G44/G442)
  - 16: `Registration_PrDocumentDate` (дата инвойса; графа 44: G44/G443)

  - 17: `Contract_PrDocumentNumber` (№ контракта-ссылки; для связи документов; графа: G44/G442)
  - 18: `Contract_PrDocumentDate` (дата контракта-ссылки; для связи документов; графа: G44/G443)

- **Стороны (местами теги “кривые” — это часть структуры Альты):**
  - 19: `Buyer_CompanyID` (ИНН покупателя; источник для мастер-данных; графы 8/9/14: G_8_6, G_9_4, G_14_4)
  - 20: `Buyer_KPPCode` (КПП покупателя; [optional], для ИП - пусто; источник для мастер-данных; графы 8/9/14: G_8_6, G_9_4, G_14_4)
  - 21: `Buyer_Name` (наименование покупателя; графы 8/9/14: G_8_NAM, G_9_NAM, G_14_NAM)
  - 22: `Buyer_PostalAddress_PostalCode` (почтовый индекс; графы 8/9/14: G_8_POS, G_9_POS, G_14_POS)
  - 23: `Buyer_PostalAddress_CountryCode` (страна покупателя alpha-2; графы 8/9/14: G_8_7, G_9_CC, G_14_CC)
  - 24: `Buyer_PostalAddress_CounryName` (страна покупателя, текст; опечатка CounryName; графы 8/9/14: G_8_50, G_9_CN,
    G_14_CN)
  - 25: `Buyer_PostalAddress_Region` (регион; графы 8/9/14: G_8_SUB, G_9_SUB, G_14_SUB)
  - 26: `Buyer_PostalAddress_City` (город; графы 8/9/14: G_8_CIT, G_9_CIT, G_14_CIT)
  - 27: `Buyer_PostalAddress_StreetHouse` (улица/дом/офис одной строкой; графы 8/9/14: G_8_STR, G_9_STR, G_14_STR)

  - 28: `Seler_Name` (продавец; опечатка Seler; источник для графы 2: G_2_NAM)
  - 29: `Seler_PostalAddress_CountryCode` (страна продавца alpha-2 из `cb:country`; графа 2: G_2_7)
  - 30: `Seler_PostalAddress_CounryName` (страна продавца, текст; опечатка CounryName; графа 2: G_2_50)
  - 31: `Seler_PostalAddress_Region` (регион продавца; графа 2: G_2_SUB)
  - 32: `Seler_PostalAddress_City` (город/район продавца; графа 2: G_2_CIT)
  - 33: `Seler_PostalAddress_StreetHouse` (улица/дом одной строкой; графа 2: G_2_STR)

  - 34: `Consignor_OrganizationName` (грузоотправитель; если отличается от продавца — для сверок/графы 2)
  - 35: `Consignor_Address_CountryCode` (страна грузоотправителя alpha-2 из `cb:country`; для сверок)
  - 36: `Consignor_Address_CounryName` (страна грузоотправителя, текст; опечатка CounryName; для сверок)
  - 37: `Consignor_Address_Region` (регион; для сверок)
  - 38: `Consignor_Address_City` (город/район; для сверок)
  - 39: `Consignor_Address_StreetHouse` (улица/дом одной строкой; для сверок)

  - 40: `Consignee_OrganizationName` (грузополучатель; обычно получатель/декларант; графы 8/9/14: G_8_NAM, G_9_NAM, G_14_NAM)
  - 41: `Consignee_OGRN` (ОГРН; графы 8/9/14: G_8_1, G_9_1, G_14_1)
  - 42: `Consignee_INN` (ИНН; графы 8/9/14: G_8_6, G_9_4, G_14_4)
  - 43: `Consignee_KPP` (КПП; [optional], для ИП - пусто; графы 8/9/14: G_8_6, G_9_4, G_14_4)
  - 44: `Consignee_Address_PostalCode` (почтовый индекс; графы 8/9/14: G_8_POS, G_9_POS, G_14_POS)
  - 45: `Consignee_Address_CountryCode` (страна alpha-2; графы 8/9/14: G_8_7, G_9_CC, G_14_CC)
  - 46: `Consignee_Address_CounryName` (страна, текст; опечатка CounryName; графы 8/9/14: G_8_50, G_9_CN, G_14_CN)
  - 47: `Consignee_Address_Region` (регион; графы 8/9/14: G_8_SUB, G_9_SUB, G_14_SUB)
  - 48: `Consignee_Address_City` (город; графы 8/9/14: G_8_CIT, G_9_CIT, G_14_CIT)
  - 49: `Consignee_Address_StreetHouse` (улица/дом/офис одной строкой; графы 8/9/14: G_8_STR, G_9_STR, G_14_STR)

- **G44 data:**
  - 50: `doc_gr44`: true
  - 51: `doc_code`: 04021 (G44/G441; относится ко всем одноименным полям ниже)
  - 52: `doc_name`: ИНВОЙС (G44/G444)
  - 53: `doc_number` (= `Registration_PrDocumentNumber`, G44/G442)
  - 54: `doc_date` (= `Registration_PrDocumentDate`, G44/G443)
...
- **Товарные позиции** (каждый элемент соответствует узлу `<InvoiceGoods>...</InvoiceGoods>`; источник для блока товаров
  ...
  - `InvoiceGoods[]`
    ...
    - 02: `GoodsDescription` (описание товара; если в инвойсе описание задано парой `<английский_текст>/<русский_текст>`,
      возьми только **РУССКИЙ ТЕКСТ**; источник для графы 31: G_31/NAME и для строк дополнения TXT/TEXT)
    - 03: `GoodsQuantity` (кол-во по строке инвойса в “основной” ед. изм.; для сверок; не использовать
  ...
  - **Неформализуемые поля.** Проверь запрет группировки товаров по кодам ТН ВЭД, 
    см. `dt_rules.md`/`aggregation_forbidden_list`.   
    - 16: `dt_item_index` (индекс товара ДТ i; см. правило группировки 7.5; если группировка запрещена индекс 1..N)
    - 17: `dt_tovg_index` (индекс позиции внутри товара j; см. правило группировки 7.5; если группировка запрещена, индекс=1)

  - _item_audit: 17

#### 3.2 Packing List / Упаковочный лист (04131)
**ОБЯЗАТЕЛЬНО:** пересчитай веса товаров согласно `dt_rules.md`/`weight_rules`. **ОБЕСПЕЧЬ СОГЛАСОВАНИЕ С ИНВОЙСОМ.** 

- **uqi_prefix:** `formalized.packing_list`
- **xml_target_root:** `AltaE2PACK`

- **Поля:**
  - 01: `GrossWeightQuantity` (общий вес брутто; для сверок; может участвовать по графам 35/38)
  - 02: `NetWeightQuantity` (общий вес нетто по упаковочному; для сверок)
...
- **Товарные/грузовые строки** (каждый элемент соответствует узлу `<Goods>...</Goods>`; это строки “по местам/грузовым
  единицам”, не по товарам ДТ). Проверь/пересчитай веса, согласно `dt_ruels.md`/`weight_rules`:
  - `Goods[]`
...
- **Поля:**
  ...
  - 31: `flight_number` (номер рейса, G21)

  - 32: `doc_gr44` (константа `true`)
  - 33: `doc_code` (константа `02017`)
  - 34: `doc_name` (константа `АВИАНАКЛАДНАЯ`)
  - 35: `doc_number` (номер в формате `<AirlineIATACode>-<DocumentNumber>`, напр. 876-41176586)
  - 36: `doc_date` (= `Registration_DateInf`)

- **Неформализуемые поля.** Получи данные от оператора, если их нельзя извлечь из первичных документов: сформируй меню 
  из `cb:customs` и `cb:svh`, попроси оператора назвать таможенный пункт на границе и СВХ или, если их нет в справочниках, 
  пусть даст коды и наименования. 

  - 37: `customs_post_code` (код таможни на границе; цель: G_29_1)
  - 38: `customs_post_name` (наименование таможни на границе; цель; G_29_2)
  - 39: `warehouse_license_number` (номер лицензии/свидетельства СВХ; цель: G_30_1)

- _audit: 39

- **Массивы:**
  - `AWBGoodsInfo_AWBGoods[]` (грузовые места/товарные позиции накладной)
...
- **Поля:**
  ...
  - 02: `PaymentModeCode` (1-электронный платеж, 2-почтовый платеж; обычно 1; системный код способа платежа)
  ...
  - 07: `ValueSpelledOut` (сумма прописью; рубли/юани/доллары прописью, копейки/фыни/центы цифрами; 
    например, "Семьдесят две тысячи шестьсот семь юаней 44 фыней)
...
- **Поля:**
  ...
  - 06: `ContractDetails_PrDocumentNumber` (№ договора на услуги/перевозку; графа 44: G44/G442)
...
- **Подписи**
  ...
  - 33: `IndividualEntrepreneur_PersonSurname` (фамилия ИП; [optional(пусто)] - если не ИП)
  - 34: `IndividualEntrepreneur_PersonName` (первый инициал/имя ИП; [optional(пусто)] - если не ИП)
  - 35: `IndividualEntrepreneur_PersonMiddleName` (второй инициал/отчество индивидуального предпринимателя; 
        [optional(пусто)]  - если не ИП)
  ...
  - 38: `SignatureDirectorChiefAccountant_Director_PersonMiddleName` (второй инициал/отчество руководителя; [optional(пусто)])
  ...
  - 41: `SignatureDirectorChiefAccountant_ChiefAccountant_PersonMiddleName` (второй инициал/отчество бухгалтера; [optional(пусто)])
...
- **Услуги** (каждый элемент соответствует узлу `<ServiceDescription>...</ServiceDescription>`):
  - `ServiceDescription[]`
    ...
    - 04: `TaxRate` (ставка налога; numeric (знак `%` НЕ указывай); для сверок/расчётов)
...
- **Поля:**
  ...
  - 04: `DocumentHead_DocumentNumber` (номер документа; графа 44: G44/G442)
    `DocumentBody_TextSection` (группирующий тег)
    - 05: `TextPara` (`link` на файл-источник; пересказ **ЗАПРЕЩЕН**)
...
- **Поля:**
  ...
  - 03: `DocumentHead_DocumentDate: <дата_инвойса>` (дата техописания, берется из инвойса; графа 44: G44/G443)
  - 04: `DocumentHead_DocumentNumber: БН` (константа; номер техописания; графа 44: G44/G442)
    `DocumentBody_TextSection` (группирующий тег)
    - 05: `TextPara` (`link` на файл-источник; если несколько файлов, перечисляются чезез `;`)
...
- Источник данных для генерации `primary.md` — файл `master_data.md`.
...
### 4.3 UNK / Уникальный номер контракта (03031)
Документ может отсутствовать. В этом случае, поставь в известность оператора и генерируй предупреждение 
в primary_overview.md.

- **uqi_prefix:** `master_data.unk`
- **Зачем:** документ для графы 44 (валютный контроль).

- **G44 data:**
  - 01: `doc_gr44`: true
  - 02: `doc_code`: 03031
  - 03: `doc_name`: УНК
  - 04: `doc_number` (= `master_data.unk.doc_number`)
  - 05: `doc_date` (= `master_data.unk.doc_date`)

- _audit: 5

### 4.4 EGRUL / Учредительные документы (04011) (в т.ч. выписка из ЕГРЮЛ)
...
### 4.5 Personal Passport / Паспорт (11001)
...
### 4.6 Letter of Attorney / Доверенность (11004)
...
### 4.7 Transport Contract / Договор транспортной экспедиции (04033)
...
### 4.8 Exemption Letter / Отказное Письмо (09023)
Может отсутствовать.
...
### 4.9 Exemption Letter (source) / Отказное Письмо (источник) (09999)
Может отсутствовать.
...
### 5.1 Описание товаров (вспомогательный, не подается)
Служебный документ сборки ДТ. Формируется **ВСЕГДА**. Нужен для графы 31. Документ формируется для каждого инвойса. 
Индексы документа и инвойса `_[n]` совпадают.
...
- **Товары в разрезе кодов ТН ВЭД,** (поле `GoodsCode` для позиций с `dt_item_index` = k):
  - `goods[]`
    - 01: `tn_ved` (ТН ВЭД)
    - 02: `description` (формируется на основе расшифровки ТН ВЭД и тех. описания, 
          см. раздел 7.6 `Правила составления описания товара для графы 31`)

  - _item_audit: 2

### 5.2 Transit Declaration / Транзитная декларация (09013)
В случае авиа перевозки документ отсутствует.
...
### 5.3 Storage Report / Отчет СВХ (ДО-1 / ДО-2) (10061/10062)
В случае авиа перевозки документ отсутствует.
...
- **Ключевые поля:**
  ...
  - 02: `actual_gross_weight` (фактический вес по весам; цель: сверка с общим брутто: графа 35: G_35_1 (контроль))
  - 03: `actual_places` (фактическое количество мест; цель: графа 6: G_6_1 (приоритет #1))
  - 04: `transport_reg_number` (номер ТС при въезде/по отчету СВХ, обычно, одной строкой номер тягача/номер прицепа; 
    [optional]; цель: сверка с графой G_18)
...
### 5.4 Storage Report Additional Sheet / Отчет СВХ дополнительный лист
...
- **Ключевые поля:**
  ...
  - 05: `transport_reg_number` (номер ТС при въезде; [optional]; цель: сверка с G_18)
...
### 5.5 Разрешительные документы (групповой шаблон)

- **uqi_prefix:** `non_formalized.regulatory_docs`
- **Зачем:** специфические для товара документы для графы 44.
- **Ключевые поля:**
  - 01: `doc_gr44: true` 

- _audit: 1

- Массив товаров, параллельный массиву в инвойсе
  - `invoice_goods[]`
    - 01: `model` (модель товара: ключ к выбору набора разрешительных документов)
    - Сертификат Соответствия (Eurasian Conformity), префикс `eac` 
      - 02: `eac_doc_code`: 01402
      - 03: `eac_doc_name`: ДЕКЛАРАЦИЯ О СООТВЕТСТВИИ
      - 04: `eac_doc_number`
      - 05: `eac_doc_date`
      - 06: `eac_date_start` (дата начала действия; цель: G44/G446)
      - 07: `eac_date_end` (дата окончания действия; цель: G44/G447)
    - Нотификация, префикс `ntf`
      - 08: `ntf_doc_code`: 10052
      - 09: `ntf_doc_name`: НОТИФИКАЦИЯ
      - 10: `ntf_doc_number`
      - 11: `ntf_doc_date`
      - 12: `ntf_date_start`
      - 13: `ntf_date_end`
    - Сертификат Происхождения (Certificate of Origin), префикс `co`
      - 14: `co_doc_code`: 06013
      - 15: `co_doc_name`: СЕРТИФИКАТ ПРОИСХОЖДЕНИЯ
      - 16: `co_doc_number`
      - 17: `co_doc_date`
      - 18: `co_date_start`
      - 19: `co_date_end`
...
### `document`: <наименование документа> (берется из заголовка шаблона, английский вариант)
  ...
  - `path`: <путь к файлу-источнику (md-файл ИЛИ оригинальный файл первички)>
  - `file_name`: <имя файла>
  - `note`: <(опционально) пояснение>
```

### 6.2 Таблицы полей документа/элемента массива
- Таблицы:
  - для каждого документа/элемента массива строится отдельная таблица;
  ...
  - и для каждого элемента массива, например:
    - `#### InvoiceGoods[3]`,
    - `#### Goods[6]`.
  - вывод таблицы **ВСЕГДА** завершается строкой `_audit`/`_item_audit`.
...
- **Нумерация и контроль потерь (Жесткие индексы):**
  ...
  - Индексы полей совпадают 1:1 в схеме и `primary.md`. AI **ОБЯЗАН** подставлять эти номера в таблицы полей  
    в том же порядке.
  - **Первой строкой** после таблицы полей документа/элемента массива AI, **ОБЯЗАН** поместить взятые из схемы строки
    (необходимо для скрипта проверки):
    - `_audit` (для документа),
    - `_item_audit` (для элемента массива).
...
### 6.6 Таблица полей документа
**ВСЕГДА** завершается строкой:
`- _audit: n`, где n - число полей документа, взятое из шаблона.

### 6.6 Массивы
Индексы массивов **ВСЕГДА 1-BASED**. Элементы массивов **ВСЕГДА** выводятся **ПОСЛЕ** таблицы полей документа. 
Каждый завершается строкой:
`_item_audit: m`, где  - число полей элемента массива, взятое из шаблона.
...
После вывода документа, включая массивы, если есть, последней строкой выводится: 
...
- _audit: <n>
<таблица элемента массива 1>
- _item_audit: <m>
...
<таблица элемента массива k>
- _item_audit: <m>
...
AI обязан:
  ...
  - **Приоритет каталога `md`:** Если папка `md` существует и содержит нужный документ, используй его. Если папки `md` 
    нет или документ в ней отсутствует, читай данные напрямую из оригинального первичного документа, применяя правила 
    доступа и оценки качества из `primary_tools.md`.
...
2. `master_data.md` (мастер-данные: ЕГРЮЛ, доверенность, паспорт, договор экспедиции)
  - `status = CD`
  - `note = master_data.md`
...
### 7.6 Правила составления описания товара для графы 31 (`dt_fields_schema.md`, п. 3.16.1)
...
- Составь краткое описание для графы 31:
  ...
  - область и условия использования,
...
### 7.12 Проверка пустых значений
Если в исходном документе смысловой блок имеет значение `ОТСУТСТВУЕТ`, перед материализацией в `primary.md` 
AI **ОБЯЗАН ПРОВЕРИТЬ ФАКТ ОТСУТСТВИЯ В ТЕКСТЕ ДОКУМЕНТА**. Если значение присутствует в тексте документа, AI берет 
его оттуда, ставит status=`pending`, формирует `note`, фиксирует противоречие в разделе нерешенных вопросов и 
**ИЗВЕЩАЕТ ОПЕРАТОРА**.
...
1) Выполни шаги из раздела `7. Начало работы` файла `primary_tools.md` (Очистка мусора и Разведка). Дополнительно 
   проверь наличие `master_data.md` и `operator_provided_data.md`. `alta\prompt\codebook.md` используется только, 
   если нет данных в вырезках справочников.

2) Определи источник данных:
  - Если папка `md` существует и описан `DOC_CONVERSION_REVIEW.md`, используй md-файлы.
  - Если папка `md` отсутствует/пуста/заполнена лишь частично, предложи оператору два варианта:
    - А) Вернуться к Этапу 0 для конвертации первички в md.
    - Б) Выполнить Этап 1 напрямую с первичных документов (PDF, DOCX, XLSX, JPEG).
    
3) Определи стратегию генерации:
  - Проверь существование папки `alta\result\<ИмяКейса>\`.
  - Подхвати готовые `primary.md`, `primary_review.md`, если есть. Запроси у оператора: нужна ли доработка или 
    генерация с нуля?
  - Если оператор не запрашивал полную генерацию, работай в режиме быстрой генерации без заполнения `note`.

4) Получи доступ в интернет, сообщи оператору: "Для улучшения качества описания товара (гр. 31, основная) языковая модель 
   будет искать в интернете расшифровку кода ТН ВЭД. Убедитесь что опция поиска включена."

5) Оцени полноту данных и документов. Если не хватает данных, обратись к оператору.

6) Проверь запрет группировки товаров по кодам ТН ВЭД, см. `dt_rules.md`/`aggregation_forbidden_list`.
...
#### 8.3.2 В цикле генерации:
  ...
  - После после выполнения директивы записи/дозаписи `primary.md`:
    - ✅ если выявлены ошибки, найди `_audit`/`_item_audit` последнего правильно сгенерированного раздела и перегенерируй
      хвост файла.
    - ✅ Пройди по всем полям со значением `ОТСУТСТВУЕТ`/"" (пусто), проверь согласно `### 7.12 Проверка пустых значений`.
    - ✅ Пройди по всем полям со статусом `pending`, проверь, что поля, действительно, нельзя извлечь/вывести.

### 8.4 Завершение (финальные проверки)
  1) **ПРОВЕРЬ ЧТО БЫЛ УЧТЕН** `operator_provided_data.md` (если есть).
  2) **ПРОВЕРЬ ПРАВИЛЬНОСТЬ ПЕРЕНОСА ДАННЫХ**, то есть, что нет рассинхрона в перенесенных полях.
  3) **ВЫЗОВИ** `alta\service\script\gen_result_full_audit.bat <путь_к_primary.md>`.
  4) **ПРОВЕРЬ, ЧТО БЫЛИ ПРИНЯТЫ ВСЕ md-ФАЙЛЫ** (или, в случае прямой генерации, все первичные документы).
  5) **ПРОВЕРЬ, ЧТО НЕ БЫЛИ СГЕНЕРИРОВАНЫ ЛИШНИЕ ДОКУМЕНТЫ**, то есть материализованы шаблоны при отсутствии 
     соответствующих md-файлов.
  6) **ПРОВЕРЬ ВЕСА В УПАКОВОЧНОМ ЛИСТЕ**, сумма весов брутто и нетто товаров должна быть равной общим весам.
  7) **ПРОВЕРЬ ЧТО ВЕСА В ИНВОЙСЕ СООТВЕТСТВУЮТ УПАКОВОЧНОМУ ЛИСТУ**.
  8) **ПРОВЕРЬ ОБЩЕЕ ЧИСЛО МЕСТ,** см. п. 7.11.
  9) **ВЫПОЛНИ:** `alta\service\script\check_pendings.bat <путь_к_primary.md>`. Скрипт найдет все pending-поля.
  10) Совместно с оператором разреши вопросы `pending` полей, если есть, выполни патчи/перегенерацию.
  11) **Зафиксируй ответы в `operator_provided_data.md`**.
  12) Сгенерируй `primary_review.md`.
...
|--------------------|--------------------------------------|------------------------|
| `cb:svh`           | Коды лицензий/наименования СВХ       |                        |
| `cb:customs`       | Коды/наимонования таможенных пунктов |                        |
...
| Код      | Наименование Т/П                    |
|----------|-------------------------------------|
| 10221010 | Т/П АЭРОПОРТ ПУЛКОВСКИЙ (ГРУЗОВОЙ)  |
| 10005020 | Т/П АЭРОПОРТ ШЕРЕМЕТЬЕВО (ГРУЗОВОЙ) |
