...
### 0.1 Завершенность обработки

Все вопросы, касающиеся первичных данных должны быть решены на этом этапе либо на основе документов, либо в диалоге
с оператором в чате AI. По результату корректируется значение поля, статус выставляется в `confirmed_operator`. Наличие 
любого поля со статусом `pending` автоматически означает невозможность перехода к следующему этапу в рабочем режиме.
Переход к следующему этапу при недостатке/конфликте данных может допускаться при отладке / формировании черновиков 
...
## 2. UQI документов и пути к полям
...
2. **Никаких догадок (Pending = Вопрос):** Если поле в документе отсутствует или не читается — `value` устанавливается 
   в `pending`. Запрещено выводить данные из косвенных ссылок без подтверждения оператора. Любой `pending` требует
   помещения в Раздел III.

3. **Документная изоляция:** На этапе 1 каждый документ обрабатывается независимо. AI не сопоставляет товарные позиции 
   между различными документами, не агрегирует их и не пытается привести к единой структуре поставки. Все расхождения 
   между документами фиксируются как конфликт и идут в Раздел III.
...
5. **Связи:** Если документ ссылается на другой (например, Инвойс на Контракт), эта связь отражается в соответствующих 
   полях.

6. На этапе 1 AI не имеет права использовать как источник фактов:
...
- Для формирования ДТ поставки автотранспортом обязательны:
  ...
  - Если в документах присутствуют явные признаки автоперевозки (номер транспортного средства, ссылка на CMR, указание
    автомобильного транспорта), AI ожидает наличие документа CMR.

Если отсутствует любой из перечисленных документов, AI устанавливает статус primary.md = pending и выносит вопрос 
в Раздел III.
...
- **Товарные позиции (goods_[n]):**
  ...
  - `trade_mark`: [xml_target: AdditionalGoodsDescription_TradeMark] (если нет - писать "ОТСУТСТВУЕТ")
  - `goods_mark`: [xml_target: AdditionalGoodsDescription_GoodsMark] (если нет - писать "ОТСУТСТВУЕТ")
...
- **Формализуемые поля:**
  ...
  - `number`: [xml_target: DocumentHead_DocumentNumber]
  - `date`: [xml_target: DocumentHead_DocumentDate]
  - `text_body`: [xml_target: TextPara] (весь технический текст или основной технический текстовый блок)
...
### 6.1. Storage Report / Отчет СВХ (ДО-1 / ДО-2) (10061/10062)
...
### 6.2. Storage Report Additional Sheet
...
## 7. РАЗДЕЛ III: Нерешенные вопросы

Здесь неразрешенные вопросы этапа 1, поля со статусом `pending`.
Раздел предназначен для чтения человеком, вопросы формулируются без жесткой формализации.
...
Если вопрос касается не определенного поля, то форма вопроса произвольная.
