...
`mapping_rules` — это process / projection layer:
значения, которые не являются фактами поставки, master data или расчетом, но нужны для корректного построения UI/XML.

Если у поля есть candidate-layer, `mapping_rules` строится поверх него, а не вместо него.
...
1. `shipment_facts`
   ...
   - факт хранения
   - номер и дата регистрации на СВХ
...
2. `mapping_rules`
   ...
   - код таможни
     ...
     | shipment_facts.warehouse_goods_location.warehouse_registry_number        | shipment_facts        | Номер документа регистрациии на СВХ         | G_30_1             | 30 Номер документа         | direct           | single       | conditional  | document                 | manual_review_only              | seen_in_ui         | conditional_for_new_dt  | verified_by_reference    | candidate-layer, не обязательно финальное значение   |
     | shipment_facts.warehouse_goods_location.warehouse_registry_date          | shipment_facts        | Дата документа регистрациии на СВХ          | G_30_DATE          | 30 Дата                    | direct           | single       | conditional  | document                 | manual_review_only              | seen_in_ui         | conditional_for_new_dt  | verified_by_reference    | candidate-layer, не обязательно финальное значение   |
...
### Важно: сначала candidate, потом final mapping rule

Сначала в `facts.md` формируется candidate-layer документа графы 44:
...
- observed candidates
  Только после этого применяется normalized mapping rule `business_role -> final code`.
...
2. Сначала формируется candidate-layer, затем выбирается final code по `business_role`.
3. Дополнительные process-коды (`09023`, `09026`, `11001`, `11004`, `04011`) не заменяют основной business-code документа.
4. Если matrix role → code не production-approved:
...
1. `payment_rule_*` — это только pattern-layer.
...
6. `mapping_rules` не являются shipment facts, но могут быть обязательны для UI/XML.
7. `needs_validation` и `manual_review_only` запрещают безусловную production-автоподстановку.
...
2. Это shipment fact, master data, calculation или mapping rule?
