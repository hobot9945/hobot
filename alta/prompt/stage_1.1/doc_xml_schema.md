...
### 1.1 Правила
  ...
  7) **Подстановка линков на `*.md` файл:**
   - **!!! ЗАПРЕЩЕНО оставлять `link:...` в сгенерированном XML.** Все линки подлежат подстановке.
   ...
   - Подставленный текст предназначен для чтения в обычном редакторе, поэтому убери все форматирование маркдауна.
  ...
  9) **Линки на `PDF`/`DOCX`/`XLXL`. Множественные линки:** 
   - Если указано несколько линков, содержимое файлов подлежит конкатенации.
   - Помести результирующий файл в `alta\work\tmp.md`, после этого исполни 7).
