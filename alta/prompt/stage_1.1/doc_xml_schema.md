...
### 1.2. Документы из stable_source (уже готовые XML)
Если документ из `primary.md` (раздел `formalized`) имеет `path` внутри `alta\stable_source\`,
то **XML не генерировать**. Вместо этого:
- Найти соответствующий XML-файл в `alta\stable_source\` (имя совпадает с `file_name` или очевидно соответствует типу документа).
- Скопировать его в `alta\stage_1.1_result\<кейс>\formalized_docs\` командой `shell_cmd copy ...`.
